...
with checkin_info as (
    select coalesce(t.location, 'UNKNOWN') as location,
           CASE WHEN c.id_check_auto != c.id_check_manual THEN 1 ELSE 0 END as mismatch,
...
select location,
       ...
       stddev_pop(false_positive) as false_positive_stddev,
       avg(false_negative) as false_negative_avg,
       stddev_pop(false_negative) as false_negative_stddev
from checkin_info
group by location
order by location;