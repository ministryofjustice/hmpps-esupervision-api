...
--changeset rob.catton:63_provider_month_mv_add_rollup_helpers splitStatements:false
DROP MATERIALIZED VIEW IF EXISTS stats_summary_provider_month;
...
provider_base AS (
  SELECT
    ...
    SUM(users_activated - users_deactivated)::BIGINT AS users_active,
...
provider_running AS (
  SELECT
    *,
    ...
    SUM(users_activated - users_deactivated) OVER (PARTITION BY provider_code ORDER BY month ASC) AS total_active_users_to_date
  FROM provider_base
),
...
all_running AS (
  SELECT
    *,
    ...
    SUM(users_activated - users_deactivated) OVER (ORDER BY month ASC) AS total_active_users_to_date
  FROM all_base
)
...
SELECT
  ...
  COALESCE(ar.total_active_users_to_date, 0)::BIGINT AS total_active_users,
  COALESCE(ar.total_deactivated_to_date, 0)::BIGINT AS total_inactive_users,
  COALESCE(ar.total_activated_to_date, 0)::BIGINT AS total_signed_up,

  -- month totals
  COALESCE(ar.users_active, 0)::BIGINT AS active_users,
  COALESCE(ar.users_activated, 0)::BIGINT AS signed_up,
  COALESCE(ar.users_deactivated, 0)::BIGINT AS inactive_users,
  ...
  CASE
    ...
    ELSE ROUND(ar.total_active_users_to_date::NUMERIC / ar.total_activated_to_date::NUMERIC, 4)
  ...
  -- month percentages
  CASE
    WHEN COALESCE(ar.users_activated, 0) = 0 THEN 0
    ELSE ROUND(ar.users_active::NUMERIC / ar.users_activated::NUMERIC, 4)
  END AS pct_active_users,

  CASE
    WHEN COALESCE(ar.users_activated, 0) = 0 THEN 0
    ELSE ROUND(ar.users_deactivated::NUMERIC / ar.users_activated::NUMERIC, 4)
...
SELECT
  ...
  COALESCE(pr.users_activated, 0)::BIGINT AS signed_up,
  COALESCE(pr.users_deactivated, 0)::BIGINT AS inactive_users,
...
/* ===========================
  ...
  -- month percentages
  CASE
    WHEN COALESCE(pr.users_activated, 0) = 0 THEN 0
    ELSE ROUND(pr.users_active::NUMERIC / pr.users_activated::NUMERIC, 4)
  END AS pct_active_users,

  CASE
    WHEN COALESCE(pr.users_activated, 0) = 0 THEN 0
    ELSE ROUND(pr.users_deactivated::NUMERIC / pr.users_activated::NUMERIC, 4)
  END AS pct_inactive_users,
