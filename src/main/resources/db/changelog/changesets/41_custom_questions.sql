...
create type question_policy as enum ('MANDATORY', 'CUSTOM');

create table question(
    id bigint generated by default as identity primary key,
    author varchar(255) not null,
    policy question_policy not null,
...
begin
    ...
    if q_policy = 'MANDATORY' then
        if lang_count <> 2 or not has_en or not has_cy then
            raise exception
                'mandatory question % must have exactly two translations: en-GB and cy-GB',
                q_id;
        end if;
    elsif q_policy = 'CUSTOM' then
        if lang_count < 1 then
            raise exception
                'custom question % must have at least one translation',
                q_id;
        end if;
    end if;
...
create constraint trigger question_localisation_check_from_question
    after insert or update of policy
    on question
...
create or replace function define_mandatory_question(
...
begin
    ...
    values ('SYSTEM', 'MANDATORY'::question_policy)
...
begin
    insert into question (author, policy)
    values (p_author, p_question_policy)
    returning id into new_question_id;
...
begin
q_mandatory_1 := define_mandatory_question(
...
q_mandatory_2 := define_mandatory_question(
...
q_mandatory_3 := define_mandatory_question(
        'SINGLE_CHOICE'::response_format,
...
create or replace function get_question_templates(p_lang text_language, p_policy question_policy)
    returns table (
                      question_id bigint,
                      question_policy question_policy,
                      question_template text,
...
begin
    return query
        ...
        from question q
                 join question_info qi on q.id = qi.question_id
        where q.policy = p_policy and qi.lang = p_lang
