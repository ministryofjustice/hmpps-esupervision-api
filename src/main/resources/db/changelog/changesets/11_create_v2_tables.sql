...
CREATE TABLE IF NOT EXISTS "offender_v2" (
    ...
    CONSTRAINT "offender_v2_status_check" CHECK (status IN ('INITIAL', 'VERIFIED', 'INACTIVE'))
);
...
CREATE TABLE IF NOT EXISTS "offender_checkin_v2" (
    ...
    "auto_id_check" VARCHAR(50),
    "manual_id_check" VARCHAR(50),
    ...
    CONSTRAINT "offender_checkin_v2_status_check" CHECK (status IN ('CREATED', 'SUBMITTED', 'REVIEWED', 'EXPIRED', 'CANCELLED')),
    CONSTRAINT "offender_checkin_v2_auto_id_check" CHECK (auto_id_check IN ('MATCH', 'NO_MATCH')),
    CONSTRAINT "offender_checkin_v2_manual_id_check" CHECK (manual_id_check IN ('MATCH', 'NO_MATCH'))
);
...
CREATE TABLE IF NOT EXISTS "generic_notification_v2" (
    ...
    CONSTRAINT "generic_notification_v2_recipient_type_check" CHECK (recipient_type IN ('OFFENDER', 'PRACTITIONER')),
    CONSTRAINT "generic_notification_v2_channel_check" CHECK (channel IN ('SMS', 'EMAIL'))
);
...
CREATE TABLE IF NOT EXISTS "job_log_v2" (
    ...
    CONSTRAINT "job_log_v2_job_type_check" CHECK (job_type IN (
        'V2_CHECKIN_CREATION',
        'V2_CHECKIN_EXPIRY'
    ))
);
