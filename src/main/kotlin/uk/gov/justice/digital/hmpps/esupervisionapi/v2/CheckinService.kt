...
class CheckinService(
  ...
  fun getCheckin(uuid: UUID, includePersonalDetails: Boolean = false): CheckinDto {
    val checkin =
    ...
    val checkinLogs = CheckinLogsDto(hint = CheckinLogsHint.SUBSET, logs = events)
  ...
  fun submitCheckin(uuid: UUID, request: SubmitCheckinRequest): CheckinDto {
   ...
   */
  private fun validateCheckinForSubmission(uuid: UUID, maybeCheckin: OffenderCheckin?): OffenderCheckin {
  ...
  fun startReview(uuid: UUID, practitionerId: ExternalUserId): CheckinDto {
    ...
    if (checkin.status != CheckinStatus.SUBMITTED && checkin.status != CheckinStatus.EXPIRED) {
      throw ResponseStatusException(
        HttpStatus.BAD_REQUEST,
  ...
  fun reviewCheckin(uuid: UUID, request: ReviewCheckinRequest): CheckinDto {
  ...
  /** Annotate a checkin */
  fun annotateCheckin(uuid: UUID, request: AnnotateCheckinRequest): CheckinDto {
    val checkin =
      checkinRepository.findByUuid(uuid).orElseThrow {
        ResponseStatusException(HttpStatus.NOT_FOUND, "Checkin not found: $uuid")
      }

    if (checkin.status != CheckinStatus.REVIEWED && checkin.status != CheckinStatus.EXPIRED) {
      throw ResponseStatusException(
        HttpStatus.BAD_REQUEST,
   ...
   */
  private fun loadCheckinForLivenessVerify(uuid: UUID): OffenderCheckin {
  ...
  private fun createCheckin(offender: Offender, dueDate: LocalDate, requestedBy: ExternalUserId): CheckinDto {
  ...
  fun sendInvite(uuid: UUID, request: CheckinNotificationRequest): CheckinDto {
    val checkin = checkinRepository.findByUuid(uuid).orElseThrow {
      ResponseStatusException(HttpStatus.NOT_FOUND, "Checkin not found: $uuid")
    }
  ...
  fun sendReminder(uuid: UUID): CheckinDto {
    val checkin =
      checkinRepository.findByUuid(uuid).orElseThrow {
        ResponseStatusException(HttpStatus.NOT_FOUND, "Checkin not found: $uuid")
      }
    ...
    val notificationAlreadySent = genericNotificationRepository.hasNotificationBeenSent(
      offender = checkin.offender,
  ...
  fun logCheckinEvent(uuid: UUID, request: LogCheckinEventRequest): UUID {
    checkinRepository.findByUuid(uuid).orElseThrow {
      ResponseStatusException(HttpStatus.NOT_FOUND, "Checkin not found: $uuid")
    }
  ...
  fun listCheckins(
    ...
    return CheckinCollectionResponse(
      pagination =
      Pagination(
 ...
 */
fun ReviewCheckinRequest.newValuesFor(status: CheckinStatus): CheckinReviewInfo {
  var errorMessage: String? = null
  var newStatus: CheckinStatus
  ...
  if (status == CheckinStatus.EXPIRED && comment.isBlank()) {
...
object CheckinRequestApplicator {
  fun OffenderCheckin.applyRequest(request: SubmitCheckinRequest, clock: Clock) {
    assert(this.status == CheckinStatus.CREATED)
    this.surveyResponse = request.survey
    this.submittedAt = clock.instant()
    this.status = CheckinStatus.SUBMITTED
  }
...
private fun OffenderCheckin.toCheckinReviewedEvent(
  contactDetails: ContactDetails?,
  clock: Clock,
  checkinWindow: Period,
  videoUrl: URL? = null,
