...
class EventAuditService(
  private val auditRepository: EventAuditRepository,
  ...
  fun recordCheckinEvent(eventType: CheckinAuditEventType, checkin: OffenderCheckin, event: ICheckinEvent) {
  ...
  fun recordCheckinEvents(eventType: CheckinAuditEventType, checkins: Iterable<Pair<OffenderCheckin, ContactDetails?>>) {
    ...
    val audits = Lists.newArrayListWithCapacity<EventAudit>(checkins.count())
   ...
   */
  fun recordCheckinCreated(checkin: OffenderCheckin, event: ICheckinEvent) = recordCheckinEvent(CheckinAuditEventType.CHECKIN_CREATED, checkin, event)

  /**
   * Record checkin submitted event
   */
  fun recordCheckinSubmitted(checkin: OffenderCheckin, event: CheckinSubmittedEvent) = recordCheckinEvent(CheckinAuditEventType.CHECKIN_SUBMITTED, checkin, event)

  /**
   * Record checkin reviewed event
   */
  fun recordCheckinReviewed(checkin: OffenderCheckin, event: CheckinReviewedEvent) = recordCheckinEvent(CheckinAuditEventType.CHECKIN_REVIEWED, checkin, event)

  /**
   * Record checkin expired event
   */
  fun recordCheckinExpired(checkins: Iterable<Pair<OffenderCheckin, ContactDetails?>>) = recordCheckinEvents(CheckinAuditEventType.CHECKIN_EXPIRED, checkins)
  ...
  private fun ICheckinEvent.toAudit(eventType: CheckinAuditEventType, checkin: OffenderCheckin): EventAudit {
  ...
  private fun OffenderCheckin.toAudit(event: CheckinAuditEventType, contactDetails: ContactDetails): EventAudit {
    val checkin = this
    return when (event) {
      CheckinAuditEventType.CHECKIN_EXPIRED -> buildAudit(
        event.name,
  ...
  companion object {
    private val LOGGER = LoggerFactory.getLogger(EventAuditService::class.java)
    ...
    fun OffenderCheckin.hoursToSubmit(): BigDecimal? = if (checkinStartedAt != null && submittedAt != null) {
      duration(Duration.between(checkinStartedAt, submittedAt))
    } else {
      null
    }

    fun OffenderCheckin.hoursToReview(): BigDecimal? = if (this.reviewStartedAt != null && this.submittedAt != null) {
      duration(Duration.between(this.submittedAt, this.reviewStartedAt))
    } else {
      null
    }

    fun OffenderCheckin.reviewDurationHours(): BigDecimal? = if (this.reviewedAt != null && this.reviewStartedAt != null) {
