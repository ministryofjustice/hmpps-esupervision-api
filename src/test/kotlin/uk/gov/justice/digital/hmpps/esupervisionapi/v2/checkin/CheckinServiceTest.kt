...
class CheckinServiceTest {
  ...
  private val offenderEventLogRepository: OffenderEventLogRepository = mock()
  ...
  fun `sendReminder - unhappy path - throws 429 when throttled by 30 min restriction to stop reminders being sent again`() {
    ...
    whenever(checkinRepository.findByUuid(uuid)).thenReturn(Optional.of(checkin))
    whenever(
      genericNotificationRepository.hasNotificationBeenSent(
        eq(offender),
  ...
  fun `submitCheckin - happy path - updates status to SUBMITTED`() {
    ...
    val request = SubmitCheckinRequest(survey = surveyData)
    ...
    assertEquals(CheckinStatus.SUBMITTED, result.status)
  ...
  fun `submitCheckin - unhappy path - checkin not found throws exception`() {
    val uuid = UUID.randomUUID()
    val request = SubmitCheckinRequest(survey = emptyMap())
  ...
  fun `submitCheckin - unhappy path - checkin past due date`() {
    ...
    val checkin = OffenderCheckin(
      uuid = uuid,
      offender = offender,
      status = CheckinStatus.CREATED, // e.g., expiry job failed and has not updated it to EXPIRED
      dueDate = dueDate.minusDays(4),
  ...
  fun `reviewCheckin - happy path & Feature ESUP_1763 TRUE - completes review and updates status`() {
    ...
    val request = ReviewCheckinRequest(
      reviewedBy = "PRACT001",
    ...
    assertEquals(CheckinStatus.REVIEWED, result.status)
  ...
  fun `reviewCheckin - happy path - saves sensitive flag as true when provided`() {
    ...
    verify(checkinPersistenceService).checkinReview(
      ...
      any(),
      eq(
        CheckinReviewInfo(CheckinStatus.REVIEWED, "Contains private health info", LogEntryType.OFFENDER_CHECKIN_REVIEW_SUBMITTED),
      ),
    )
  }
  ...
  fun `reviewCheckin - happy path - saves sensitive flag as false when not provided`() {
    ...
    verify(checkinPersistenceService).checkinReview(
      ...
      any(),
      eq(
        CheckinReviewInfo(CheckinStatus.REVIEWED, "Test note", LogEntryType.OFFENDER_CHECKIN_REVIEW_SUBMITTED),
      ),
    )
  }
  ...
  fun `annotateCheckin - happy path - updates checkin sensitive flag as true`() {
    ...
    val request = AnnotateCheckinRequest(
      updatedBy = "PRACT001",
  ...
  fun `recordLivenessClientFailure - rejects a checkin that's already submitted`() {
    val uuid = UUID.randomUUID()
    val checkin = createCreatedCheckin(uuid).apply { status = CheckinStatus.SUBMITTED }
    whenever(checkinRepository.findByUuid(uuid)).thenReturn(Optional.of(checkin))
  ...
  private fun createCreatedCheckin(uuid: UUID, livenessEnabled: Boolean = false) = OffenderCheckin(
    uuid = uuid,
    offender = createOffender(),
    status = CheckinStatus.CREATED,
    dueDate = LocalDate.now(clock),
