...
class EventDetailServiceTest {
  ...
  private lateinit var service: EventDetailService

  @BeforeEach
  fun setUp() {
    service = EventDetailService(checkinRepository, eventLogRepository, proxyLinkCreator, appConfig, surveyExpansionsConfig)
  }
  ...
  inner class CheckinNotesFormatting {
    ...
    fun `formats checkin notes for missed checkin event`() {
      val uuid = UUID.randomUUID()
      val offender = createOffender(UUID.randomUUID())
      val checkin = createCheckin(uuid, offender, status = CheckinStatus.EXPIRED)
      whenever(checkinRepository.findByUuid(uuid)).thenReturn(Optional.of(checkin))

      val logEntry = OffenderCheckinLogEntryDto(
        UUID.randomUUID(),
  ...
  fun `adds annotations`() {
    val uuid = UUID.randomUUID()
    val offender = createOffender(UUID.randomUUID())
    val checkin = createCheckin(uuid, offender, status = CheckinStatus.SUBMITTED)
    whenever(checkinRepository.findByUuid(uuid)).thenReturn(Optional.of(checkin))
    val logEntry = OffenderCheckinLogEntryDto(
      UUID.randomUUID(),
      notes = "Some note",
  ...
  inner class SensitiveFlagMapping {
    ...
    fun `includes sensitive flag for reviewed events`() {
      val uuid = UUID.randomUUID()
      val offender = createOffender(UUID.randomUUID())
      val checkin = createCheckin(uuid, offender, status = CheckinStatus.REVIEWED).apply {
        sensitive = true
      }
      whenever(checkinRepository.findByUuid(uuid)).thenReturn(Optional.of(checkin))
    ...
    fun `includes sensitive flag for annotated events`() {
      ...
      val checkin = createCheckin(checkinUuid, offender, status = CheckinStatus.REVIEWED).apply {
        sensitive = true
      }
    ...
    fun `sensitive flag is false when not explicitly set in check in reviewed`() {
      val uuid = UUID.randomUUID()
      val offender = createOffender(UUID.randomUUID())
      val checkin = createCheckin(uuid, offender, status = CheckinStatus.REVIEWED)
      whenever(checkinRepository.findByUuid(uuid)).thenReturn(Optional.of(checkin))
    ...
    fun `sensitive flag is false when not explicitly set in check in annotated`() {
      ...
      val checkin = createCheckin(checkinUuid, offender, status = CheckinStatus.REVIEWED)
  ...
  private fun createOffender(uuid: UUID) = Offender(
    uuid = uuid,
    crn = "X123456",
